...
type_defs = gql(
   ...
   type Mutation{
      ...
      add_attendees(firstname: String!, lastname: String!, mailid: String!): Attendees
      }
   """
)
...
@query.field("attendees")
def attendees(*_):
   return [attendees.to_json() for attendees in Attendees.query.all()]
...
@mutation.field("add_attendees")
def add_attendees(_,info,firstname,lastname,mailid):
   attendee = Attendees(firstname=firstname, lastname=lastname, mailid=mailid)
   attendee.save()
...
class Attendees(db.Model):
   mailid = db.Column(db.String(80), primary_key=True)
